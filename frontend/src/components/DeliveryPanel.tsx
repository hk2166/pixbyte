...
function SdTab({ jobId, stats }: { jobId: string | null; stats: JobStats | null }) {
  const sizeKb = stats?.total_kb ?? '—';
    ...
    <div className="delivery-content">
      <div className="delivery-left">
        <div className="stats-grid">
          <div className="stat-cell">
            <div className="stat-key">// size</div>
            <div className="stat-val">{sizeKb} KB</div>
          </div>
          <div className="stat-cell">
            <div className="stat-key">// filename</div>
            <div className="stat-val">display.oled</div>
          </div>
        </div>
        <div className="info-box">
          <span>ℹ</span>
          Copy <strong>display.oled</strong> to the root of your FAT32/FAT16 SD card. Set MODE_SD in the firmware sketch.
        </div>
        <div className="btn-row">
          <a
            href={jobId ? downloadUrl(jobId) : '#'}
            download="display.oled"
            style={{ textDecoration: 'none' }}
          >
            <button className="btn-download" disabled={!jobId || !stats}>
              ↓ DOWNLOAD (.oled)
            </button>
          </a>
        </div>
      ...
      <div className="delivery-right">
        <pre className="code-block">
          <span className="cmt">{'// SD card wiring (SPI)'}</span>{'\n'}
          <span className="kw">MOSI</span> → GPIO <span className="num">23</span>{'\n'}
          <span className="kw">MISO</span> → GPIO <span className="num">19</span>{'\n'}
          <span className="kw">CLK</span>  → GPIO <span className="num">18</span>{'\n'}
          <span className="kw">CS</span>   → GPIO <span className="num">5</span>{'\n\n'}
          <span className="cmt">{'// set in firmware'}</span>{'\n'}
          <span className="kw">#define</span> MODE_SD{'\n'}
          <span className="kw">#define</span> SD_FILENAME <span className="str">"/display.oled"</span>
        </pre>
      </div>
    </div>
  );
...
function WifiTab({ jobId, frameCount, fps }: {
  jobId: string | null;
  frameCount: number;
  fps: number;
}) {
  const metaUrl = jobId ? streamMetaUrl(jobId) : `${BASE}/api/stream/JOB_ID/meta`;
  const frameUrl = jobId ? `${BASE}/api/stream/${jobId}/frame/0` : `${BASE}/api/stream/JOB_ID/frame/0`;
  const serverPort = getServerPort(BASE);
    ...
    <div className="delivery-content">
      <div className="delivery-left">
        <div className="stats-grid">
          <div className="stat-cell">
            <div className="stat-key">// frames</div>
            <div className="stat-val">{frameCount || '—'}</div>
          </div>
          <div className="stat-cell">
            <div className="stat-key">// fps</div>
            <div className="stat-val">{fps || '—'}</div>
          </div>
        </div>
        <div className="info-box">
          <span>ℹ</span>
          ESP32 fetches frames over HTTP. Keep server and ESP32 on the same LAN. Set MODE_WIFI in firmware.
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span className="label-dim">// meta endpoint</span>
          <code style={{ fontSize: 10, color: 'var(--cyan-dim)', wordBreak: 'break-all' }}>{metaUrl}</code>
          <span className="label-dim" style={{ marginTop: 4 }}>// frame endpoint</span>
          <code style={{ fontSize: 10, color: 'var(--cyan-dim)', wordBreak: 'break-all' }}>{frameUrl}</code>
        </div>
      </div>
      <div className="delivery-right">
        <pre className="code-block">
          <span className="cmt">{'// firmware config'}</span>{'\n'}
          <span className="kw">#define</span> MODE_WIFI{'\n'}
          <span className="kw">#define</span> WIFI_SSID <span className="str">"your_ssid"</span>{'\n'}
          <span className="kw">#define</span> WIFI_PASS <span className="str">"your_pass"</span>{'\n'}
          <span className="kw">#define</span> SERVER_IP <span className="str">"192.168.x.x"</span>{'\n'}
          <span className="kw">#define</span> SERVER_PORT <span className="num">{serverPort}</span>{'\n'}
          <span className="kw">#define</span> JOB_ID <span className="str">"{jobId ?? 'JOBID'}"</span>
        </pre>
      </div>
    </div>
  );
